...
const CardDetail: React.FC = () => {
  ...
  if (!card) {
    return (
      <div {...getXRProps()} className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
  ...
  return (
    <div {...getXRProps()} className="card-detail-container min-h-screen relative overflow-hidden">
      ...
      {/* Header */}
      <header {...getXRProps()} className="relative bg-slate-800/90 backdrop-blur-lg border-b border-slate-700/50">
      ...
      <div {...getXRProps()} className="relative max-w-[1400px] mx-auto px-6 py-8">
        <div {...getXRProps()} className="flex flex-col lg:flex-row gap-8 items-start">
          {/* Card Image - Fixed size container */}
          <div {...getXRProps()} className="card-detail-image lg:w-[500px] flex-shrink-0">
            <div {...getXRProps()} className="sticky top-8">
              <div {...getXRProps()} className="relative aspect-[686/1000] max-w-[500px] mx-auto bg-slate-800/90 backdrop-blur-xl rounded-xl overflow-hidden shadow-2xl shadow-slate-900/30 border border-slate-600/50">
                <img
                  {...getXRProps()}
                  src={card.imageUrl}
                  alt={card.name}
                ...
                {!card.inStock && (
                  <div {...getXRProps()} className="absolute inset-0 bg-red-900/80 backdrop-blur-sm flex items-center justify-center border-2 border-red-600/50">
                    <div {...getXRProps()} className="bg-red-600/90 text-red-100 px-6 py-3 rounded-lg font-bold text-sm shadow-xl border border-red-500/30">
                      UNAVAILABLE
                    </div>
                  </div>
                )}
          ...
          <div {...getXRProps()} className="card-detail-info flex-1 space-y-6">
            {/* Title and Key Info */}
            <div {...getXRProps()} className="space-y-3">
              ...
              <h1 {...getXRProps()} className="text-2xl lg:text-3xl font-bold text-slate-100 leading-tight">
                {card.name}
              </h1>

              <div className="flex items-center gap-4">
                <div {...getXRProps()} className={`inline-flex items-center px-3 py-1 rounded-md text-xs font-bold bg-gradient-to-r ${getRarityColor(card.rarity || '')} text-white shadow-lg border`}>
                  {card.rarity}
                </div>
                <div {...getXRProps()} className={`text-sm font-semibold ${card.inStock ? 'text-emerald-400' : 'text-red-400'}`}>
            ...
            {/* Price Section */}
            <div {...getXRProps()} className="bg-slate-800/80 backdrop-blur-xl rounded-lg p-4 border border-slate-600/50 shadow-xl">
              ...
              <p {...getXRProps()} className="text-xs text-emerald-400 mt-2 font-medium">✓ FREE shipping on orders over $35</p>
            </div>

            {/* Card Specifications */}
            <div {...getXRProps()} className="bg-slate-800/60 backdrop-blur-xl rounded-lg p-4 border border-slate-600/50 shadow-xl space-y-3">
              <h3 className="text-lg font-bold text-slate-200 tracking-wider">CARD DETAILS</h3>
              <dl className="space-y-2">
                <div {...getXRProps()} className="flex justify-between py-2 border-b border-slate-700/50">
            ...
            {/* Description */}
            <div {...getXRProps()} className="bg-slate-800/60 backdrop-blur-xl rounded-lg p-4 border border-slate-600/50 shadow-xl space-y-2">
            ...
            {/* Purchase Box */}
            <div {...getXRProps()} className="bg-slate-800/80 backdrop-blur-xl rounded-lg p-5 border border-slate-600/50 shadow-xl space-y-4">
              <div className="space-y-3">
                ...
                <div className="space-y-1">
                  <p {...getXRProps()} className="text-emerald-400 font-medium flex items-center gap-2 text-sm">
                    <span>✓</span> FREE delivery Tomorrow
                  </p>
                  <p className="text-slate-400 text-xs">Order within 2 hrs 30 mins</p>
                </div>

                <div {...getXRProps()} className={`text-sm font-bold ${card.inStock ? 'text-emerald-400' : 'text-red-400'}`}>
                  {card.inStock ? '✓ AVAILABLE' : '✗ UNAVAILABLE'}
                </div>
              </div>
              ...
              <div {...getXRProps()} className="space-y-3">
                ...
                <button
                  disabled={!card.inStock}
                  {...getXRProps()}     className={`w-full py-2 px-4 rounded-md font-semibold text-sm transition-all duration-200 ${
                    card.inStock
              ...
              <div {...getXRProps()} className="pt-4 border-t border-slate-700/50">
                <div className="space-y-2">
                  <div {...getXRProps()} className="flex items-center gap-3 text-slate-400">
